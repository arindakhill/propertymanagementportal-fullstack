...
const HouseDetails = () => {
    ...
    if (user) {
      try {
        const response = await axios.get(
          `http://backend:8080/api/v1/properties/favourite-properties`,
          {
            headers: {
        ...
        setIsLiked(
          response.data.some((property) => property.id === parseInt(propertyId))
        )
      } catch (error) {
  ...
  const likeProperty = async () => {
    if (user) {
      try {
        await axios.post(
          `http://backend:8080/api/v1/properties/${propertyId}/like`,
          {},
          {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem('token')}`,
  ...
  const unlikeProperty = async () => {
    if (user) {
      try {
        await axios.post(
          `http://backend:8080/api/v1/properties/${propertyId}/unlike`,
          {},
          {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem('token')}`,
    ...
    <>
      ...
      >
        <Box>
          ...
          <Text fontSize="20px">
            {searchedHouse.address.country} {searchedHouse.address.city}{' '}
            {searchedHouse.address.street} {searchedHouse.address.zipCode}
          </Text>
        </Box>
        ...
        <HStack>
          ...
          >
            {'for ' + searchedHouse.listingType}
          </Text>
          <Text px="3" borderRadius="full" bg="purple.300">
            {searchedHouse.address.country}
          </Text>
          <Text px="3" borderRadius="full" bg="teal.300">
            {searchedHouse.address.state}
          </Text>
          <Text px="3" borderRadius="full" bg="teal.300">
            {searchedHouse.address.city}
          </Text>
